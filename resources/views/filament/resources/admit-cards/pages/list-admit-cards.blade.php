...
<x-filament-panels::page>
    @if ($classes->isEmpty())
        <x-filament::empty-state
            ...
            description="Add active classes first to manage admit cards."
        />
    @else
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            @foreach ($classes as $class)
                @php
                    ...
                    $url    = AdmitCardResource::getUrl('class-admit-cards', ['class' => $class->id]);
                @endphp
                ...
                >
                    ...
                    <div class="flex items-center gap-2 px-3 pt-3 pb-2">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <x-filament::badge
                                size="sm"
                                :color="$class->total_admit_cards > 0 ? 'primary' : 'gray'"
                            >
                                {{ $class->total_admit_cards }} {{ Str::plural('card', $class->total_admit_cards) }}
                            </x-filament::badge>
                        </div>
                    </div>
                    ...
                    <div class="flex-1 space-y-1 px-3 pb-2">
                        @if ($class->total_admit_cards > 0)
                            @foreach ($statusMeta as $key => $meta)
                                @php $count = $class->admitCardStats->get($key, 0); @endphp
                                @if ($count > 0)
                        ...
                        @else
                            <div class="rounded border border-dashed border-gray-200 px-2 py-3 text-center dark:border-gray-700">
                                <p class="text-[11px] text-gray-400 dark:text-gray-500">No admit cards yet</p>
                            </div>
                        @endif
                    </div>
                        ...
                        @if ($class->pending_count > 0)
                            <div>
                                <p class="text-xs font-bold text-warning-600 dark:text-warning-400">
                                    {{ $class->pending_count }} {{ Str::plural('card', $class->pending_count) }}
                                </p>
                                <p class="text-[10px] leading-tight text-gray-500 dark:text-gray-400">
                                    Pending
                                </p>
                            </div>
                        @elseif ($class->total_admit_cards > 0)
                            <div>
                                <p class="text-xs font-bold text-success-600 dark:text-success-400">All generated</p>
                                <p class="text-[10px] leading-tight text-gray-500 dark:text-gray-400">No pending</p>
                            </div>
                        @else
