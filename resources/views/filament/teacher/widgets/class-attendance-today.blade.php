<x-filament-widgets::widget>
    <div class="rounded-xl bg-white p-3 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 sm:p-4">
        <h2 class="mb-3 flex items-center gap-x-1.5 text-sm font-semibold text-gray-900 dark:text-white sm:text-base">
            ...
            {{ __("Today's Attendance — My Classes") }}
        </h2>
                    ...
                    <div class="min-w-0">
                        ...
                        @if ($class->is_marked)
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                <span class="text-success-600 dark:text-success-400">{{ __('Present') }} {{ $class->present_today }}</span>
                                ·
                                <span class="text-danger-600 dark:text-danger-400">{{ __('Absent') }} {{ $class->absent_today }}</span>
                            </p>
                        @else
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __(':count Students', ['count' => $class->student_profiles_count]) }}</p>
                        @endif
                    </div>
